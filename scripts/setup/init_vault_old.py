...
# Import VaultClient if available, otherwise use direct Vault commands
try:
    from common.vault import VaultClient
    VAULT_CLIENT_AVAILABLE = True
except ImportError:
    VAULT_CLIENT_AVAILABLE = False
    print("Note: Using direct Vault CLI commands instead of VaultClient")


def run_vault_command(command: str) -> str:
    """Run a Vault CLI command."""
    try:
        # Set the correct Vault address and token, then run the command
...
def verify_secrets():
    ...
    try:
        ...
        binance_result = run_vault_command("kv get secret/api_keys/binance")
        if "api_key" in binance_result and "secret_key" in binance_result:
            print("✓ Binance credentials retrieved successfully")
        else:
            print("✗ Failed to retrieve Binance credentials")
        
        # Test agent configuration
        research_result = run_vault_command("kv get secret/agents/research")
        if "data_sources" in research_result:
            print("✓ Research agent configuration retrieved successfully")
        else:
            print("✗ Failed to retrieve research agent configuration")
        
        # Test database credentials
        db_result = run_vault_command("kv get secret/databases/default")
        if "host" in db_result and "password" in db_result:
            print("✓ Database credentials retrieved successfully")
        else:
            print("✗ Failed to retrieve database credentials")
        
        # List all secrets
        list_result = run_vault_command("kv list secret")
        if list_result:
            print(f"✓ Found secrets: {list_result.strip()}")
