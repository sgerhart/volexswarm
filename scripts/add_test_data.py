#!/usr/bin/env python3
"""
Add test price data to database for autonomous AI testing.
"""
...
def add_test_price_data():
    """Add test price data to the database."""
    try:
        ...
        for i, price in enumerate(prices):
            ...
            if i % 100 == 0:
                print(f"   - Added {i+1}/{len(prices)} price data points")
        
        session.commit()
        session.close()
        
        print(f"✅ Successfully added {len(prices)} price data points to database")
...
if __name__ == "__main__":
    print("Adding Test Price Data for Autonomous AI")
    print("=" * 40)
    
    if add_test_price_data():
