"""
VolexSwarm Agentic News Sentiment Agent - Main Entry Point
...
logger = get_logger("agentic_news_sentiment_main")

class AgenticNewsSentimentService:
    """Service wrapper for the agentic news sentiment agent."""
    ...
    async def start(self):
        """Start the agentic news sentiment service."""
        try:
            logger.info("Starting Agentic News Sentiment Service...")
            
            # Initialize the agentic news sentiment agent
            self.agent = AgenticNewsSentimentAgent()
            await self.agent.initialize()
            
            self.running = True
            logger.info("Agentic News Sentiment Service started successfully")
            
        except Exception as e:
            logger.error(f"Failed to start Agentic News Sentiment Service: {e}")
            raise
    
    async def stop(self):
        """Stop the agentic news sentiment service."""
        try:
            logger.info("Stopping Agentic News Sentiment Service...")
            self.running = False
            
            if self.agent:
                await self.agent.shutdown()
            
            logger.info("Agentic News Sentiment Service stopped successfully")
            
        except Exception as e:
            logger.error(f"Error stopping Agentic News Sentiment Service: {e}")
...
service = AgenticNewsSentimentService()
...
# Create FastAPI app
app = FastAPI(title="VolexSwarm News Sentiment Agent", lifespan=lifespan)



@app.get("/health")
async def health_check():
    """Health check endpoint for the News Sentiment Agent."""
    try:
...
async def root():
    """Root endpoint."""
    return {"message": "VolexSwarm News Sentiment Agent", "status": "running"}
